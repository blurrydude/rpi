...
if __name__ == "__main__":
    ...
    while running is True:
        ...
        if mode <= 2:
            ...
            #time.sleep(5)
            mosquittoMessage("mqtt_rgb "+str(myip).split(' ')[1].replace("b'","")+" alive at "+str(round(time.time())))
        elif mode == 3 or mode == 4:
            if j + 1 > 255:
                j = 0
                mosquittoMessage("bqtt_rgb "+str(myip).split(' ')[1].replace("b'","")+" alive at "+str(round(time.time())))
            else:
                j = j + 1
            if mode == 3:
                fill(wheel(j))
